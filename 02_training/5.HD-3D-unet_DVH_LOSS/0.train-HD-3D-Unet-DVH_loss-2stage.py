...
def total_loss(y_true, y_pred, roi_masks, meter=None):
    ...
    total = (base_loss * 3.5) + (dvh_penalty * 0.5) # 3.5 0.5

    print(f"base : {base_loss}, dvh : {dvh_penalty}")
    if meter:
...
def main():
    ...
    if os.path.exists(stage_1_pth) and os.path.exists(save_file):
    ...
    elif os.path.exists(stage_1_pth) == True and os.path.exists(save_file) == False:
        print("load 1stage weight")
        try:
            model.load_state_dict(torch.load(stage_1_pth, map_location=device))
            print("# Weights loaded successfully!")
        except Exception as e:
            print(f"# Error loading weights: {e}")
            print("# Starting from scratch.")
    ...
    writer = SummaryWriter(log_dir=f"runs/HD_3D_UNet_stage2_{time.strftime('%Y%m%d_%H%M%S')}")
