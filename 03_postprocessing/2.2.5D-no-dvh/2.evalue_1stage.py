...
# --- Configuration ---
# 경로 설정
PRED_DIR = os.path.join(os.environ.get("WBI_OUTPUT_ROOT", "./outputs"), "prediction_npy_2_5D_unet_SimAM")
...
def save_comparison_figure(ct_slice, roi_dict_2d, true_dose_slice, pred_dose_slice, diff_slice, gamma_maps, out_path):
    """선량 비교 및 3종 감마 맵 시각화 (3x3 배열)"""
    ...
    # 5. Gamma 1%1mm
    im4 = axes[1, 1].imshow(gamma_maps[0], cmap='RdYlGn_r', vmin=0, vmax=2); axes[1, 1].set_title("Gamma (1%/1mm, Rel)"); axes[1, 1].axis("off")
    fig.colorbar(im4, ax=axes[1, 1], fraction=0.046, pad=0.04)
    
    # 6. Gamma 2%2mm
    im5 = axes[1, 2].imshow(gamma_maps[1], cmap='RdYlGn_r', vmin=0, vmax=2); axes[1, 2].set_title("Gamma (2%/2mm, Rel)"); axes[1, 2].axis("off")
    fig.colorbar(im5, ax=axes[1, 2], fraction=0.046, pad=0.04)

    # Row 3: 감마 3 및 나머지
    # 7. Gamma 3%3mm
    im6 = axes[2, 0].imshow(gamma_maps[2], cmap='RdYlGn_r', vmin=0, vmax=2); axes[2, 0].set_title("Gamma (3%/3mm, Rel)"); axes[2, 0].axis("off")
...
def process_single_patient(pred_file):
    ...
    ct_slice_mid = np.transpose(ct_contour_4d, (2, 0, 1, 3))[z_mid, :, :, 0]
    save_comparison_figure(ct_slice_mid, {n: m[z_mid] for n, m in roi_masks_orig.items()}, gt_slice, pred_slice, diff_slice, gamma_maps_2d, os.path.join(p_vis_dir, "dose_comparison_with_gamma.png"))
...
def main():
    ...
    df_all[cols].to_csv("case_dvh_evaluation_result_2.5D.csv", index=False)

    with open("inference_result_2.5D.md", "w") as md_file:
    ...
    print(f"\nEvaluation complete. Report saved to 'inference_result_2.5D.md'.")
