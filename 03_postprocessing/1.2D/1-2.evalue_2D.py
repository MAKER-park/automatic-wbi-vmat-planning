...
# --- Configuration ---
# 경로 설정
PRED_DIR = os.path.join(os.environ.get("WBI_OUTPUT_ROOT", "./outputs"), "prediction_2D_npy")
...
VIS_OUTPUT_DIR = "evaluation_visuals" # 시각화 결과 저장 폴더
...
def calculate_ssim_3d(true_vol, pred_vol, data_range):
    """3D SSIM 점수 계산"""
...
def analyze_and_print_results(results_list, group_name, md_file_handle, vis_dir):
    ...
    for gt_col, pred_col in zip(gt_cols, pred_cols):
        ...
        p_value = wilcoxon(gt_series, pred_series).pvalue if len(gt_series) > 1 and len(pred_series) > 1 else np.nan
...
def main():
    ...
    for pred_file in tqdm(pred_files, desc="Evaluating predictions"):
        ...
        # DICE 및 SSIM은 원본 해상도에서 계산
        dice_threshold = RX_DOSE * 0.5
        patient_results['DICE_gt'] = 1.0
        patient_results['DICE_pred'] = calculate_dice_3d(gt_dose_gy_orig, pred_dose_gy_orig, dice_threshold)
        patient_results['SSIM_gt'] = 1.0
        patient_results['SSIM_pred'] = calculate_ssim_3d(gt_dose_gy_orig, pred_dose_gy_orig, data_range=TARGET_DOSE)
    ...
    print(f"Summary report saved to 'Readme.md'.")
