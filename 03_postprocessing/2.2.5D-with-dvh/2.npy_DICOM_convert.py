...
# 기본 설정
# =========================
pred_npy_dir = os.environ.get("WBI_PREDICTION_ROOT", os.path.join(os.environ.get("WBI_OUTPUT_ROOT", "./outputs"), "./prediction_npy_2_5D_unet_SimAM"))
...
def extract_patient_key_from_string(name_str: str) -> str:
    s = str(name_str).strip().upper()

    m = re.search(r"(?:^|[^A-Z0-9])(\d{3})\s*(W[LR])(?:$|[^A-Z0-9])", s)
    if m:
        return f"{m.group(1)}{m.group(2)}"

    m = re.search(r"(?:^|[^A-Z0-9])(\d+)\s*(W[LR])(?:$|[^A-Z0-9])", s)
    if m:
...
def extract_patient_key_from_dicom(ds: pydicom.Dataset) -> str:
    """
    DICOM Dataset에서 PatientName 우선, 안되면 PatientID에서
    000WL/000WR 형태의 key를 추출.
    """
    for tag in ["PatientName", "PatientID"]:
        if hasattr(ds, tag):
            try:
                return extract_patient_key_from_string(getattr(ds, tag))
            except ValueError:
                continue
    raise ValueError("DICOM에서 환자번호(WL/WR 포함)를 찾지 못했습니다.")
...
def extract_patient_key_from_npy_filename(filename: str) -> str:
    """
    NPY 파일명에서 000WL/000WR 형태의 key 추출
    - 언더스코어(_)도 경계로 인정되도록 \b 대신 (?:^|[^A-Z0-9]) 사용
    """
    s = os.path.basename(filename).upper()

    # 1) 가장 안전: "숫자3자리 + WL/WR" 토큰을 구분자 기준으로 찾기
    #    (^ 또는 비영숫자) + (ddd)(WL/WR) + ($ 또는 비영숫자)
    m = re.search(r"(?:^|[^A-Z0-9])(\d{3})(W[LR])(?:$|[^A-Z0-9])", s)
    if m:
        return f"{m.group(1)}{m.group(2)}"

    # 2) 혹시 3자리 아닌 경우도 허용 (예: 98WL -> 098WL)
    m = re.search(r"(?:^|[^A-Z0-9])(\d+)(W[LR])(?:$|[^A-Z0-9])", s)
    if m:
        return f"{m.group(1).zfill(3)}{m.group(2)}"

    raise ValueError(f"NPY 파일명에서 WL/WR 포함 환자키를 찾지 못했습니다: {filename}")
...
def build_patient_dicom_map(dicom_root: str):
    ...
    def _scan_folder_list(folder_list, modality_label):
        for folder in folder_list:
            ...
            pid = extract_patient_key_from_dicom(ds)
...
def build_prediction_index_single_folder(pred_dir: str):
    """
    pred_dir 안의 *.npy만 읽어서
    model_type(폴더명) → patient_id → [npy_paths...] 형태로 반환
    """
    pred_dir = os.path.normpath(pred_dir)
    if not os.path.isdir(pred_dir):
        raise RuntimeError(f"예측 NPY 폴더가 없습니다: {pred_dir}")

    npy_files = sorted(glob.glob(os.path.join(pred_dir, "*.npy")))
    print(f"[INFO] pred_npy_dir: {pred_dir}")
    print(f"[INFO] npy 파일 수: {len(npy_files)}")

    index = defaultdict(lambda: defaultdict(list))
    model_type = os.path.basename(pred_dir)
    ...
    print(f"[INFO] model_type: {model_type}, 환자 수: {len(index[model_type])}")
    return model_types, index
...
def create_rtdose_dataset(ct_ds: pydicom.Dataset,
    ...
    # model tage 설명 및 tps 구분을 위해서 추가
    today_str = time.strftime("%Y_%m_%d")
    ds.SeriesDescription = f"{model_type}_{prediction_label}_{today_str}"[:64]
...
def main():
    ...
    model_types, pred_index = build_prediction_index_single_folder(pred_npy_dir)
